"""
SK-Res2Net 백본 + Dynamic MLP 결합 모델.

본 모듈은 SK2Res2Net(SK attention + Res2Net) 백본에 Dynamic MLP를 결합한
주력 모델을 제공한다. ResNet보다 더 강력한 feature 추출 능력을 가진다.

SK2Res2Net의 핵심 개선점:
  1. Res2Net: Bottleneck 내부에서 다중 스케일 feature를 추출
     (3x3 컨볼루션을 여러 개의 작은 그룹으로 분할)
  2. SK (Selective Kernel): 입력에 따라 다른 스케일의 커널을 선택
     (어텐션 메커니즘으로 스케일 가중치를 학습)

이 백본의 출력(2048차원)에 Dynamic MLP로 메타데이터를 융합하여
최종 분류를 수행한다.
"""
import math
...
def build_plugin_layer(*args, **kargs):
    """플러그인 레이어 빌더 (현재 미사용, 확장용 placeholder)."""
    pass
...
class SK2Res2Net(ResNet):
    """
    SK2Res2Net 백본 + Dynamic MLP 융합 모델.

    ResNet을 상속하여 Res2Net 다중 스케일 구조와 SK attention을 적용하고,
    Dynamic MLP 컴포넌트(loc_net, loc_att)를 추가한다.

    Args:
        scales (int): Res2Net의 스케일 수. Default: 4
        base_width (int): 각 스케일의 기본 채널 너비. Default: 32
    ...
    def __init__(self,
        ...
        # ── Dynamic MLP 컴포넌트 ──────────────────────────────────────
        # loc_net: 메타데이터(4차원) → 256차원 임베딩 인코더
        # loc_att: 이미지 feature(2048) + loc 임베딩 → 융합 feature (FusionModule)
        self.loc_net = FCNet(num_inputs=args.mlp_cin, num_classes=args.mlp_d, num_filts=256)
        self.loc_att = get_dynamic_mlp(2048, args)

    def make_res_layer(self, **kwargs):
        """SK2Layer(Res2Net + SK attention)를 생성하여 반환한다."""
    ...
    def forward(self, x, loc):
        """
        이미지와 메타데이터를 입력받아 클래스 로짓을 출력한다.

        Args:
            x: 이미지 텐서 [B, 3, 224, 224]
            loc: 메타데이터 벡터 [B, mlp_cin(4)]

        Returns:
            클래스 로짓 [B, num_classes]
        """
        # ── 백본: 이미지 → 2048차원 feature ────────────────────────────
        # deep_stem: 3개의 3x3 컨볼루션으로 7x1을 대체하여 더 부드러운 다운샘플링
        if self.deep_stem:
            x = self.stem(x)
        else:
            x = self.conv1(x)
        ...
        # 4개의 residual stage를 통과하며 점진적으로 채널 증가, 해상도 감소
        for i, layer_name in enumerate(self.res_layers):
            res_layer = getattr(self, layer_name)
            x = res_layer(x)

        # Global Average Pooling: [B, 2048, H, W] → [B, 2048, 1, 1] → [B, 2048]
        x = self.avgpool(x)
        x = torch.flatten(x, 1)

        # ── Dynamic MLP: 메타데이터 융합 ──────────────────────────────
        # 메타데이터를 256차원 임베딩으로 인코딩
        loc_fea = self.loc_net(loc)
        # 이미지 feature에 메타데이터 임베딩을 동적 융합 (잔차 연결)
...
def sk2res2net101(logger, args):
    """
    SK2Res2Net-101 + Dynamic MLP 모델을 생성한다.

    본 프로젝트의 주력 모델. ResNet-101보다 더 강력한 feature 추출 능력을 가진다.
    depth=101: [3, 4, 23, 3] Bottle2neck 블록 구성
    scales=4: Res2Net 다중 스케일 (3x3 컨볼루션을 4개 그룹으로 분할)
    base_width=26: 각 스케일의 채널 너비
    ratio=4: SK attention의 축소 비율
    """
