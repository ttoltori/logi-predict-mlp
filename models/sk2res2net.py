"""
SK2Res2Net 백본 모듈 (image-only 비교용).

본 모듈은 Dynamic MLP를 결합하지 않은 순수 SK2Res2Net 백본을 제공한다.
Dynamic MLP 버전(sk2res2net_dynamic_mlp)과 성능을 비교하기 위해 사용한다.

SK2Res2Net = Res2Net(다중 스케일 feature) + SK(Selective Kernel attention)
사용 시 --image_only 플래그와 함께 사용해야 한다.
"""
import math
